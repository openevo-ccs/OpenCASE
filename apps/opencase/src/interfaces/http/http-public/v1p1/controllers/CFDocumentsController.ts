    ...
    try {
      ...
      if (resolved.version !== '1.1') {
        return res.status(409).json(StatusInfoFormatter.internalError(
          `CFDocument '${sourcedId}' exists in CASE v1p0. Use GET /ims/case/v1p0/CFDocuments/${sourcedId} (and related v1p0 endpoints).`
        ))
      }

      // Access control: unauthenticated requests only see public frameworks
      if (!(req as any).isAuthenticated && !this.store.isDocumentPublic(resolved.tenantId, '1.1', sourcedId)) {
