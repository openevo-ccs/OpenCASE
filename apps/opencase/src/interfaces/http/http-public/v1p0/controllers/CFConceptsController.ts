...
export class CFConceptsControllerV1p0 {
  constructor (
    private readonly getCFConcept: GetCFConcept,
  ...
  getById = async (req: Request, res: Response) => {
    try {
      ...
      const resolved = this.store.resolveDefinitionGlobal('CFConcepts', sourcedId)
      if (!resolved) return res.status(404).json(StatusInfoFormatter.notFound('The requested CFConcept was not found.'))

      const result = await this.getCFConcept.execute({
        tenantId: resolved.tenantId,
        caseVersion: resolved.version,
        sourcedId
      })

      if (!result) return res.status(404).json(StatusInfoFormatter.notFound('The requested CFConcept was not found.'))
